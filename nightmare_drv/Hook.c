...
BOOLEAN ReadFromUm(PVOID dest, PVOID src, SIZE_T size, PEPROCESS umProcess) {
	SIZE_T returnSize = 0;
	if (NT_SUCCESS(MmCopyVirtualMemory(umProcess, src, PsGetCurrentProcess(), dest, size, KernelMode, &returnSize)) && returnSize == size) {
		return TRUE;
	}
...
BOOLEAN WriteToUm(PVOID dest, PVOID src, SIZE_T size, PEPROCESS umProcess) {
	SIZE_T returnSize = 0;
	if (NT_SUCCESS(MmCopyVirtualMemory(PsGetCurrentProcess(), src, umProcess, dest, size, KernelMode, &returnSize)) && returnSize == size) {
		return TRUE;
	}
...
PVOID GetModuleBaseAddress(PCHAR name) {
	...
	if (STATUS_INFO_LENGTH_MISMATCH != status) {
		DPRINT("[NIGHTMARE DRV] ZwQuerySystemInformation for size failed: %x", status);
		return addr;
	}

	PSYSTEM_MODULE_INFORMATION modules = ExAllocatePool(NonPagedPool, size);
	if (!modules) {
		DPRINT("[NIGHTMARE DRV] Failed to allocate %d bytes for modules", size);
		return addr;
	}

	if (!NT_SUCCESS(status = ZwQuerySystemInformation(SystemModuleInformation, modules, size, 0))) {
		DPRINT("[NIGHTMARE DRV] ZwQuerySystemInformation failed: %x", status);
...
BOOLEAN SetupHook()
{
	...
	if (!kernelBase) {
		DPRINT("[NIGHTMARE DRV] Failed to find kernel base");
		return FALSE;
	}
	...
	if (!addr) {
		DPRINT("[NIGHTMARE DRV] Failed to find the function pointer");
		return FALSE;
	}
